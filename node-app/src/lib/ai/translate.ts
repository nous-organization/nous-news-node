...
export type TranslationResult = {
	...
	errors?: string[]; // optional array of errors that occurred
};
...
async function getTranslatorPipeline(srcLang: string, targetLang: string) {
	if (!translatorPipeline) {
		...
		if (srcLang === "en" || targetLang === "en") {
			...
			translatorPipeline = await getPipeline("translation", "m2m100");
		}
	}
...
// Translate text using AI, with token truncation and error handling.
// ---------------------------------------------------------
export async function translateContentAI(
	...
	try {
		...
		for (const sentence of sentences) {
			...
			try {
				...
				if (translator.modelId.includes("m2m100")) {
					const forcedBOS = translator.tokenizer.getLangId(tgtLang);
				...
				console.warn("[translateContentAI] sentence failed:", err);
				errors.push((err as Error).message);
				translated.push(sentence); // fallback to original sentence
			}
		}
		...
		console.warn("[translateContentAI] Failed:", (err as Error).message);
		return {
			status: "error",
			content,
...
/**
 * Translate only translation strings (with fallbacks):
 */
export async function translateMultipleTitlesStrings(
	titles: string[],
	targetLanguage?: string,
	batchSize = 10,
): Promise<string[]> {
	const results = await translateMultipleTitlesAI(
		titles,
		targetLanguage,
		batchSize,
	);
	return results.map((r) => r.translation ?? r.content);
}
